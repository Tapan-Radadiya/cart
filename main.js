...
productList.forEach((data) => {
    ...
    itemQuant.setAttribute("type", "text")
    itemQuant.value = curQuant.quant
    itemQuant.disabled = true
    itemQuant.setAttribute("id", `txtQty${data.id}`)
    itemQuant.setAttribute("class", "itemQuant")

    const itemIdText = document.createTextNode(`ID : ${data.id}`)
    const itemTitleText = document.createTextNode(`Title : ${data.title}`)
    const itemPriceText = document.createTextNode(`Price : ${data.price}`)
    const itemDescriptionText = document.createTextNode(`Description : ${data.description}`)
    const itemCategory = document.createTextNode(`Category : ${data.category}`)
    const rateText = document.createTextNode(`Rate : ${data.rating.rate}`)
    const countText = document.createTextNode(`Count : ${data.rating.count}`)


    itemId.appendChild(itemIdText)
    itemTitle.appendChild(itemTitleText)
    price.appendChild(itemPriceText)
    description.appendChild(itemDescriptionText)
    category.appendChild(itemCategory)
    rate.appendChild(rateText)
    count.appendChild(countText)

    const addToCart = document.createElement("button")
    addToCart.setAttribute("id", `btnCart${data.id}`)
    ...
    increBtn.setAttribute("id", `btnIncrement${data.id}`)
    increBtn.setAttribute("class", "countBtn")
    increBtn.addEventListener("click", addItem)

    const decreBtn = document.createElement("button")
    decreBtn.textContent = "-"
    decreBtn.setAttribute("class", "countBtn")
    decreBtn.setAttribute("id", `btnDecrement${data.id}`)
    decreBtn.addEventListener("click", removeItem)
    ...
    function addItemToCart(event) {
        ...
        cartListIncrement.setAttribute("id", `cartListIncrement${itemId}`)
        cartListIncrement.setAttribute("class", "countBtn")
        cartListIncrement.addEventListener("click", addCartItem)

        const cartListDecrement = document.createElement("button")
        cartListDecrement.textContent = "-"
        cartListDecrement.setAttribute("id", `cartListDecrement${itemId}`)
        cartListDecrement.setAttribute("class", "countBtn")
        cartListDecrement.addEventListener("click", removeCartItem)

        const removeItem = document.createElement("button")
        removeItem.setAttribute("id", `btnRemove${itemId}`)
        removeItem.textContent = "Remove Item"
        removeItem.setAttribute("class", "removeFromCartBTN")
        removeItem.addEventListener("click", removeItemFromCart)

        const totalPrice = document.createElement("p")
        totalPrice.setAttribute("id", `TotalPrice${itemId}`)
        totalPrice.innerText = curItemCount * data.price
        totalCart.innerText = totalCartPrice

        cartItemDiv.append(cartItemImage, cartItemTitle, cartItemDesc, cartItemPrice, cartListIncrement, cartItemQuant, cartListDecrement, removeItem, totalPrice)
        cart.appendChild(cartItemDiv)
        cart.appendChild(totalCart)


        // Add or delete item from cart
        function addCartItem(event) {
            let curItem = parseInt(cartItemQuant.value)
            let curPrice = parseFloat(totalPrice.innerText)
            let actualPrice = parseFloat(data.price)
            let curItemQuant = parseInt(itemQuant.value)
            curItemQuant += 1
            itemQuant.value = curItemQuant

            totalCartPrice = parseFloat(totalCart.innerText)
            if (curItem >= 1) {
                cartListDecrement.disabled = false
            }

            curPrice = (curPrice + actualPrice).toFixed(2)
            curPrice = parseFloat(curPrice)
            totalCartPrice = (totalCartPrice + data.price).toFixed(2)
            totalCart.innerText = totalCartPrice
            totalPrice.innerText = curPrice
            curItem += 1
            cartItemQuant.value = curItem

        }
        function removeCartItem(event) {
            let curItem = parseInt(cartItemQuant.value)
            if (curItem == 1) {
                cartListDecrement.disabled = true
                return
            }
            let curPrice = parseFloat(totalPrice.innerText)
            let actualPrice = parseFloat(data.price)
        ...
        function removeItemFromCart() {
            const curItem = document.getElementById(`cartItem${itemId}`)
            const curItemPrice = document.getElementById(`TotalPrice${itemId}`)
            addToCart.disabled = false
            increBtn.disabled = false
            decreBtn.disabled = false
            let removeItemPrice = parseFloat(curItemPrice.innerText).toFixed(2)
            totalCartPrice = totalCartPrice - removeItemPrice
            totalCart.innerText = totalCartPrice
            curItem.remove()
        }
    }
